...
<script>
    ...
    XuatNhapTonAPP.createData = function () {
        ...
        function tonghop() {
            ...
            if (dowloaded >= 5) {
                for (var i = 0; i < data_list.length; i++) {
                    ...
                    btn_filter.html('<i class="fa fa-search"></i> Tìm kiếm');
                }
        ...
        download_data_where(urls.url_apis.select, {
            ...
            select: "WHERE products_list.code=product_logs.product_list AND STR_TO_DATE(product_logs.create_at,'%d/%m/%Y') AND product_logs.branch LIKE '" + txt_branch.jqxDropDownList('val') + "' AND products_list.branch LIKE '" + txt_branch.jqxDropDownList('val') + "'  GROUP BY products_list.code",
        ...
        download_data_where(urls.url_apis.select, {
            table: "product_logs,products_list",
                select: "WHERE products_list.code=product_logs.product_list AND product_logs.number<0 AND STR_TO_DATE(product_logs.create_at,'%d/%m/%Y') AND STR_TO_DATE(product_logs.create_at,'%d/%m/%Y') AND product_logs.branch LIKE '" + txt_branch.jqxDropDownList('val') + "' AND products_list.branch LIKE '" + txt_branch.jqxDropDownList('val') + "'  GROUP BY(product_logs.product_list)",
